...
export default function LandingPage() {
	return (
		<div className="relative min-h-screen overflow-x-clip bg-[radial-gradient(1200px_600px_at_50%_-10%,rgba(99,102,241,0.25),transparent),radial-gradient(1000px_500px_at_90%_10%,rgba(59,130,246,0.18),transparent)] text-slate-900">
			<Decor />

			{/* HERO */}
			<section aria-labelledby="hero-title" className="container mx-auto px-4 sm:px-6 lg:px-8 py-20 md:py-28">
				<div className="mx-auto max-w-6xl text-center">
					<div className="inline-flex items-center gap-2 rounded-full border border-slate-200/80 bg-white/60 px-3 py-1 text-xs font-medium text-slate-600 shadow-sm backdrop-blur supports-[backdrop-filter]:backdrop-blur">
						<Sparkles className="h-3.5 w-3.5 text-violet-600" aria-hidden />
						ShopMate AI — belanja dibantu AI, cepat & akurat
					</div>

					<h1 id="hero-title" className="mt-5 text-4xl md:text-6xl font-extrabold leading-tight tracking-tight">
					...
					<div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
						<Button asChild size="lg" className="font-semibold shadow-sm hover:shadow md:gap-1">
							<Link href="/products" aria-label="Jelajahi Produk">
								Jelajahi Produk <ArrowRight className="ml-2 h-4 w-4" aria-hidden />
							</Link>
						</Button>
						<Button asChild size="lg" variant="outline" className="font-semibold border-slate-300 hover:bg-white/60 backdrop-blur">
					...
					{/* small feature row (keeps content, improves layout) */}
					<div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
						<HeroCard icon={<ShoppingCart className="h-4 w-4" aria-hidden />} title="Katalog lengkap" desc="Cari cepat, filter pintar" />
						<HeroCard icon={<MessageSquare className="h-4 w-4" aria-hidden />} title="Chat rekomendasi" desc="Saran sesuai preferensi" />
						<HeroCard icon={<Wand2 className="h-4 w-4" aria-hidden />} title="Banding instan" desc="Plus-minus jelas" />
						<HeroCard icon={<Shield className="h-4 w-4" aria-hidden />} title="Privasi aman" desc="Data kamu terlindungi" />
					</div>
				</div>
			</section>
			...
			{/* KEUNGGULAN AI */}
			<section aria-labelledby="adv-title" className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
